    ...
    if baris_aktual_validasi.empty:
        ...
        if df_sebelum_validasi[df_sebelum_validasi["Kabupaten/Kota"] == selected_city].empty:
            ...
            fig_validasi.update_layout(
                title=f"Data Aktual vs Prediksi Tahun {TAHUN_VALIDASI} - {selected_city}",
                yaxis_title="Ton/Tahun",
                template="plotly_dark",
                height=460,
                paper_bgcolor="rgba(0,0,0,0)",
            ...
            st.caption(
                f"Grafik ini menunjukkan seberapa dekat prediksi model dengan kenyataan untuk tahun "
                f"{TAHUN_VALIDASI}. Model dilatih hanya menggunakan data sebelum tahun {TAHUN_VALIDASI} "
                f"(tanpa 'mengintip' data aktual {TAHUN_VALIDASI}), sehingga selisihnya mencerminkan "
                f"akurasi prediksi yang sesungguhnya."
            )
